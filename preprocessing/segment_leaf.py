...
This module is an engineering baseline for controlled PlantVillage images.
It is intentionally not presented as expert-validated plant pathology
segmentation. The RGB image remains available for classifier input.
"""
...
def make_leaf_mask(rgb: np.ndarray) -> np.ndarray:
    ...
    # Green/chlorotic tissue is usually more chromatic than neutral backgrounds.
    # Combine HSV saturation with Lab a* to avoid relying on grayscale intensity.
    sat_mask = (s >= 35).astype(np.uint8) * 255
    green_a_mask = (a <= 150).astype(np.uint8) * 255

    # Preserve darker leaf tissue if it is still reasonably chromatic.
    chromatic = ((s >= 25) & (v >= 25)).astype(np.uint8) * 255
    combined = cv2.bitwise_or(sat_mask & green_a_mask, chromatic & sat_mask)
    ...
    # Boundary holes inside a leaf should not remove leaf area.
...
def estimate_lesions(rgb: np.ndarray, leaf_mask: np.ndarray, random_state: int = 42) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """Estimate healthy/chlorotic/necrotic regions using Lab K-Means (k=3).

    Cluster labels are mapped by centroid heuristics. This is a proxy mask for
    severity and Grad-CAM localization evaluation, not expert ground truth.
    """
    ...
    # L, a, b captures brightness and green/yellow/blue-red chromatic cues.
    kmeans = KMeans(n_clusters=3, random_state=random_state, n_init=10)
    labels = kmeans.fit_predict(pixels)
    centers = kmeans.cluster_centers_

    # Necrosis proxy: darkest cluster among the leaf clusters.
    necrotic_label = int(np.argmin(centers[:, 0]))

    remaining = [i for i in range(3) if i != necrotic_label]
    # Chlorosis proxy: relatively yellow/high b* among non-necrotic clusters.
